module Users
  ##
  # This controller contains overrides the standard Devise SessionController to
  # support multi-factor authentication.
  #
  # Each action has a comment describing it's function. Also see
  # doc/multi_factor_authentication.md for a more verbose description of the
  # sequence of events for MFA.
  #
  class SessionsController < Devise::SessionsController
    ...
    # since middleware runs before any controller actions, resulting in the user
    ...
    # does, we render a response to request their MFA code, which then submits
    # to the #validate_otp action.
    #
    def create
      # Once we are inside the controller action, we _do_ want to allow params
      # authentication, otherwise Devise won't authenticate the user from params.
      allow_params_authentication!

      # Tell Devise not to track the user after any call to set_user.  We set
      # this because we don't want to track a successful sign in until after MFA
      # verification.
    ...
    def resource_from_otp_identifier(otp_identifier)
      GlobalID::Locator.locate_signed(otp_identifier, for: RESOURCE_OTP_TOKEN_PURPOSE, only: User)
    end
